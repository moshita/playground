...
//var signalling = new WebSocket("ws://127.0.0.1:8777");
//var signalling = new WebSocket("ws://ec2-35-163-145-148.us-west-2.compute.amazonaws.com:8777");
var signalling = new WebSocket("wss://test.moshita.xyz");

hangupButton.disabled = true;
callButton.onclick = call;
hangupButton.onclick = hangup;

var pc1 = new RTCPeerConnection(null, {'optional': []});
...
//  pc2.onaddstream = gotRemoteStream;
  ...
  pc1.onnegotiationneeded = negotiationNeeded;
...
function gotDescription1(desc) {
  ...
  pc1.setLocalDescription(desc).then(
    function() {
      ...
      signalling.send(JSON.stringify({'type': desc.type, 'sdp': desc.sdp}));
      */
      /*
...
signalling.onmessage = function(message) {
	var dataJson = JSON.parse(message.data);
  if(dataJson.sdp != undefined) {
 		// SDP
    if(dataJson.type == 'offer') {
      incomingcall();
      pc1.setRemoteDescription(new RTCSessionDescription(dataJson)).then(
          function() {
...
function iceCallback1(event) {
  if (event.candidate) {
    trace('Vanilla ICE ' + event.candidate);
	  //signalling.send(JSON.stringify({'candidate': event.candidate}));
  /*
    pc2.addIceCandidate(
  ...
  */
    ...
    trace('Canilla ICE Local Description is \n' + desc);
    desc.sdp = forceChosenAudioCodec(desc.sdp);
    signalling.send(JSON.stringify({'type': desc.type, 'sdp': desc.sdp}));
  }
}
